  ...
  return (
    ...
    >
      ...
      >
        ...
        <span className="font-caveat text-2xl bg-clip-text text-transparent bg-gradient-to-r from-[#00D1B2] to-[#E6EDF3]">
          ZHM
        </span>
      </motion.div>
      ...
      <div className="hidden md:flex gap-8 text-sm font-medium text-gray-300">
        {navLinks.map((link, i) => (
          ...
          >
            {link}
            <span className="absolute left-0 bottom-0 w-0 h-[2px] bg-gradient-to-r from-[#00D1B2] to-[#E6EDF3] transition-all duration-300 group-hover:w-full"></span>
          </motion.a>
        ))}
      </div>
      ...
      <motion.a
        ...
        className="hidden md:flex bg-gradient-to-r from-[#1B2A4A] to-[#00D1B2] text-white px-5 py-2 rounded-lg items-center gap-2 shadow-md shadow-[#0B1220]/50 hover:shadow-lg hover:shadow-[#1B2A4A]/60 hover:scale-105 transition"
        whileHover={{ y: -2 }}
      >
        My Resume
      </motion.a>
          ...
          >
            {navLinks.map((link, i) => (
              <a
                key={i}
                href={`#${link.toLowerCase().replace(/\s+/g, "")}`}
                className="text-lg font-medium text-gray-200 hover:text-[#00D1B2] transition"
            ...
            <a
              ...
              target="_blank"
              rel="noopener noreferrer"
              className="bg-gradient-to-r from-[#1B2A4A] to-[#00D1B2] text-white px-5 py-2 rounded-lg shadow-md hover:shadow-lg transition"
              onClick={() => setIsOpen(false)}
            >
              My Resume
            </a>
